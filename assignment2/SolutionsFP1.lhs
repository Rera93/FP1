...
The results observed indicate that the Int type has a limitation on the range of possible values due to 
...
and can "hold any value" up to the machine's memory size, thus the result from the function product is 
correct because it is within the range of the type Integer.
The machine yelds zero because the value stored in the bits of type Int represent the number zero due to
the overflow caused by the result of product [1 .. 66] :: Int.
